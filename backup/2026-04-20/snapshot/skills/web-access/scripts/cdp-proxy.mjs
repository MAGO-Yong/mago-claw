...
async function discoverChromePort() {
  ...
  for (const port of commonPorts) {
    ...
    if (ok) {
      ...
      // 尝试通过 HTTP API 获取真实 webSocketDebuggerUrl（含 UUID）
      try {
        const wsPath = await new Promise((resolve, reject) => {
          http.get(`http://127.0.0.1:${port}/json/version`, { timeout: 3000 }, (res) => {
            ...
            res.on('end', () => {
              try {
                ...
                const match = url.match(/\/devtools\/browser\/.+/);
        ...
        if (wsPath) {
          console.log(`[CDP Proxy] 获取到 wsPath: ${wsPath}`);
          return { port, wsPath };
        }
      } catch { /* 继续，wsPath 为 null */ }
