...
description: 按 app/service 名称、产品线或业务线搜索 Xray 告警规则列表，支持 PQL 告警和服务告警两类。当用户需要查询某个服务/产品线/业务线下有哪些告警规则时触发。
version: 1.0.0
metadata:
  category: alarm
  subcategory: rule
...
`app` 与 `service` 可混用：传入具体 service 名称（如 `xrayaiagent-service-diagnosis`）时使用 `--app` 参数。
...
如需查看某条规则的完整配置（PromQL 表达式、触发条件等），使用 `alarm-rule-detail` skill，传入对应 `id`。
