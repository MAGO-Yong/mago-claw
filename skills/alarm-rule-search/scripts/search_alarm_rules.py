...
API_URL = "https://xray-ai.devops.xiaohongshu.com/open/skill/alarm/rule/all/v2"


def fetch(filter_key: str, filter_val: str, page: int, size: int) -> dict:
    body = json.dumps({"page": page, "size": size, filter_key: filter_val}).encode()
...
def format_rule(r: dict) -> dict:
    ...
    return {
        "ruleId": r.get("ruleId"),
        "type": r.get("datasourceType"),
        "name": r.get("name"),
        "alarmTarget": r.get("alarmTarget"),
        "level": r.get("level"),
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "搜索 Xray 服务告警规则列表（中间件/客户端/RPC 等阈值类告警）。\n"
        ...
        epilog=(
            ...
            "\n"
            "输出字段：ruleId / type(datasourceType: middleware/transaction/problem/jvm/host) / name / alarmTarget / level / status / modifyTime / modifier\n"
