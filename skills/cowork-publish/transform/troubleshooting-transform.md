...
| 现象 | 根因 | 修复 |
|---|---|---|
| `bash install.sh` 报 `guard-transform 工具不完整：缺少 ...` | install.sh 没在 `skills/seal/` 目录下 | 确保用源仓库内的脚本：`bash <repo>/skills/seal/install.sh` |
| 装完 Claude Code 没识别新 skill | Claude Code 仅在会话启动时扫描 `~/.claude/skills/`，无运行时 reload 机制 | **退出当前会话**（CTRL+D 或 `/exit`）→ 重新跑 `codewiz-cc` 开新会话即可识别。`/exit` 后立刻重开也行；不存在 "reload skills" / "刷新 skills" 这种运行时指令（之前文档误写过，已纠正） |
| `$SKILL_DIR/.guard_transform_home` 不存在 | 安装被中断 / 目录被破坏 | `bash <repo>/skills/seal/install.sh --force` 强制重装 |
| 装在自定义目录后 Claude Code 找不到 | Claude Code 默认只扫 `~/.claude/skills/` | 装回默认目录，或自行配置 Claude Code 扫描路径 |
...
| 现象 | 根因 | 修复 |
|---|---|---|
| `framework: unknown` | 检测规则没覆盖（`detect_rules.json`） | 看 `<state>/stack.json`，手动改 framework 字段 + `bin/guardx transform <src> -y` 续跑（checklist 命中跳过 detect） |
| 识别错（如把 monorepo 识成单仓） | profile 评分模糊 | 同上：直接改 stack.json 后 `bin/guardx transform <src> -y` 续跑 |
...
| `codewiz-cc` 鉴权失败 / 401 | OAuth token 过期 | `codewiz-cc login` 重新登录 |
| `codewiz-cc` 超时 / 长时间无心跳 | 模型慢或网络抖动 | claude skill 默认已 1800s 超时；如仍不够：`export GUARD_LLM_TIMEOUT=3600 GUARD_LLM_HEARTBEAT=300; source $GUARD_TRANSFORM_HOME/default_env.sh; bin/guardx transform <src> --resume` |
| LLM 没改文件 | tool denied / 权限交互被拒 | `cat <state>/llm-*.log` 看具体被拒哪个 tool；guardx 已默认带 `--dangerously-skip-permissions` |
| LLM 改了但漏改关键文件 | prompt 第 N 段不准 | 看 `<state>/_retry-*.md`，找漏的部分；自动 retry 已 ≤3 次。**用户**可主动换更强后端再续跑（**agent 不会替你切**，避免悄悄换模型）：`export GUARD_LLM=codewiz GUARD_LLM_MODEL='codewiz/Claude-4.6-opus(thinking)'` → `source $GUARD_TRANSFORM_HOME/default_env.sh` → `bin/guardx transform <src> --resume` |
| LLM 反复同样的错 | autofix 死循环 | `--autofix-max 3` 限次或 `--no-autofix` 直接看原错 |
| 想跳过 LLM 调试流水线骨架 | — | `--skip-llm`；或显式覆盖默认 `export GUARD_LLM=mock; SKIP_LLM=1 bin/guardx transform ...` |
...
| 现象 | 根因 | 修复 |
|---|---|---|
| 缺端口 / start.sh 没有 `0.0.0.0:3000` | 模板渲染脚本 bug 或 stack.json 没标对端口 | `cat <work>/start.sh`；改完模板后 `bin/guardx transform <src> --from-stage 30 --resume`（仅高级用法保留 `--from-stage`） |
| db.properties 字段不全 | 工程类型没识别为 has_db | 改 `<state>/stack.json` 的 `has_db: true` 后 `bin/guardx transform <src> --from-stage 30 --resume` |
...
| `verify_db_properties` 失败 | 6 个标准 key 缺失 | 看 `<work>/db.properties`，对照 README 的 PG 配置规范补齐 |
| `verify_health_endpoint` 超时 | 应用启动慢或 health 路径不对 | 看 `<state>/smoke-*.log`；调整 health 路径或加大超时 |
| `verify_no_url_absolute` 拦到 HTML `href="/foo"` | ⚠️ **verifier 旧版 bug**：与规范 `prompts/23_fix_paths.md §2` 冲突——规范明确说 HTML 里 `<a href="/foo">` 保留 `/`，由 router 注入前缀。verifier 已修复为只拦 `href="http://..."` / `href="//host/..."` 这两类真违规 | 升级到本仓库新版 `verifiers/verify_no_url_absolute.sh` 即可；若仍复现说明你的 skill 安装目录是旧版，需 `bash <repo>/skills/seal/install.sh --force` 重装 |
...
**带伤通过**（仅调试用，正式产出不要用）：
```bash
GUARD_STRICT=0 bin/guardx transform <src> --resume    # 或 --no-strict
```
...
| 现象 | 根因 | 修复 |
|---|---|---|
| zip 体积超大 | node_modules / dist 未被 ignore | 检查 `<work>/.guardignore`（如有）；补排除规则后 `bin/guardx pack <src>` |
...
| 整个工程从头再来 | `bin/guardx clean <src> -y && bin/guardx transform <src>` |
| 不调 LLM 看流水线骨架 | `bin/guardx transform <src> --skip-llm` |
...
3. **手工接管**：进 `<work>` 自己改完、自己 build；然后跑 `bin/guardx verify <src>` 单独验证 → 跑 `bin/guardx pack <src>` 直接打包
4. **用户主动换更强后端 / 模型**（覆盖 default_env.sh 的 claude 默认；**agent 不会替你做这一步**）：
   ...
   "$GUARD_TRANSFORM_HOME/bin/guardx" transform <src> --resume
   ```
