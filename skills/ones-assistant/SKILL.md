---
name: ones-assistant
description: "ones 发布平台操作助手。当用户需要进行以下操作时触发：查询有权限的应用/服务、查看服务部署组信息、查询可用镜像版本、查询或浏览发布流程模板、触发 SIT 或 staging 环境发布、查询发布进度或发布详情、查询发布历史、查询 Pod 状态、诊断发布卡住或异常、查询我的发布列表、查询我的测试项目、创建测试项目（创建测试泳道/创建泳道环境）。Use when: deploying services to ones platform, checking deploy status, querying workload groups, browsing image tags, or diagnosing deploy failures."
---
...
本 skill 封装了 ones 发布平台的所有 Agent Tool 接口，支持：

1. **发布流程**：从服务查询、镜像选择、发布创建，到发布状态追踪和异常诊断的完整发布闭环
2. **测试项目（泳道）管理**：创建、查询和管理个人测试环境，支持多应用多服务部署
...
| 查我的测试项目列表 | `list-projects` |
| 查测试项目详情 | `describe-project <projectName>` |
| 创建测试项目 | `create-project --alias ... --lane ... --expire-days ... --applications ...` |
...
当用户需要创建测试项目（泳道/个人环境）时，按以下步骤执行：

#### Step 1 — 收集必要信息

用户需要提供以下信息来创建项目：
- `alias`：项目别名（人类可读名称）
- `lane`：泳道名（系统内唯一，小写字母+数字+短横线）
- `expire_days`：过期天数（建议 1-30）
- `applications`：应用配置数组（包含应用名、服务名、镜像 tag、部署组名等）

**如果用户未提供完整信息，按以下方式推荐**：

若用户未提供应用或服务信息，查询用户有权限的应用和服务列表：
...
展示列表供用户选择，或根据用户上下文自动推荐。

#### Step 2 — 查询服务的部署组和镜像版本（按需）

当用户确定了要部署的服务后，如果用户未提供部署组名或镜像 tag，查询该服务的部署组和可用镜像版本：
...
返回结果包含：
- **部署组列表**（workloadGroupName）：用于 `create-project` 的 `workload_group_name` 参数
- **镜像版本列表**（tag）：用于 `create-project` 的 `image_tag` 参数

展示列表供用户选择，或根据用户上下文自动推荐。

#### Step 3 — 构建应用配置并创建项目

根据用户提供的信息（或推荐的信息），构建 `applications` 参数（JSON 数组），然后创建项目：

```bash
python scripts/ones_deploy.py create-project \
  --alias "项目别名（人类可读名称）" \
  --lane "泳道名（系统内唯一，小写字母+数字+短横线）" \
  --expire-days <天数> \
  --applications '[
    {
      ...
      "children": [
        {
          "name": "服务名（applicationChildName/service）",
          "image_tag": "镜像tag",
          "workload_group_name": "部署组名",
          "replicas": 1
        }
      ]
    }
  ...
  [--description "项目描述（可选）"] \
  [--env "sit 或 staging（默认 sit）"]
```

**必填参数说明**：
- `alias`：项目别名，人类可读，如 "我的联调项目"
- `lane`：泳道名，系统内唯一，建议格式 `my-feature-123`（小写字母+数字+短横线）
- `expire_days`：过期天数，建议 1-30，到期自动回收
- `applications`：应用配置数组，**每个应用下的服务必须包含以下字段**：
  - `name`：服务名（applicationChildName）
  - `image_tag`：镜像 tag
  - `workload_group_name`：部署组名
...
**可选参数**：
- `description`：项目描述
- `env`：部署环境，默认 `sit`，可选 `sit` 或 `staging`

脚本返回包含 `project_name` 和 ones 页面 `link` 的 JSON。
...
```bash
# 查询我的测试项目列表
python scripts/ones_deploy.py list-projects

# 查询特定项目的详情（含应用、服务、部署状态等）
python scripts/ones_deploy.py describe-project <projectName>
```
...
6. **测试项目创建信息补全**：创建测试项目时，如果用户未提供完整的应用、服务、部署组或镜像信息，
   应通过 `my-apps`、`my-services`、`service-info`、`images` 等命令查询并推荐给用户选择。
   确保 `applications` 参数中的 `name`、`image_tag`、`workload_group_name` 等字段与实际系统中的值一致。

7. **遇到 agent-platform 未识别错误**：若调用接口时提示 agent-platform 未识别或相关鉴权失败，
