...
class DecisionEngine:
    ...
    def _display_metrics(self, metrics: dict, template: Optional[str]):
        ...
        for key, data in metrics.items():
            ...
            if count > 0:
                vals = [float(v[1]) for v in data if isinstance(v, (list, tuple)) and len(v) >= 2 and v[1]]
                if vals:
                    avg_val = sum(vals) / len(vals) * 100
                    print(f"  {key}: {count}个数据点, 均值={avg_val:.2f}%")
                else:
                    print(f"  {key}: {count}条数据")
            else:
                print(f"  {key}: 无数据")
    ...
    def _evaluate_condition(self, condition: dict, node: dict) -> dict:
        ...
        for check in checks:
            ...
            node_data = self.context.get(node.get("id"), {})
            # 简化评估：如果节点有数据则认为条件可评估
        ...
        all_met = all(r.get("met", False) for r in results) if results else True
        return {"result": all_met, "details": results, "metrics": self.context.get(node.get("id"), {})}
