...
> 每题展开说明，突出亮点与问题，归因到 SKILL 设计层面。

### ✅/⚠️/❌ {题号}（{类型}）— {得分}/5.0

**Query**：`{原始 query}`  
**耗时**：{n}s  
**AI 链路 Trace**：⏳ 等待补充（AI trace SKILL 接入后自动填充）

**Agent 回答摘要**：
> {50-100字摘要，抓住回答的核心内容}

**✅ 做得好的地方**：
- {具体描述，例如：正确识别了用户意图，调用了 xx 能力而非直接查日志}
- {如无亮点可省略此项}

**❌ 存在的问题**：
- {具体描述，例如：参数识别错误，将 pod 名传入了 subApplication 字段}
- **SKILL 归因**：{说明这是 SKILL 的哪个设计问题，例如：SKILL 的 nl_to_query 转换逻辑未区分 pod 名和服务名的格式差异，需在参数提取模块增加 pod 名格式校验（格式：xxx-xxxxx）}

**分项得分**：意图识别 {x} / Skill选择 {x} / 推理过程 {x} / 完整性 {x} / 准确性 {x} / 交付 {x} / 速度 {x}

---

（每题一节，按 S1→S2→...→B1→...→C1→C2 顺序排列）

---

## Bad Patterns（系统性问题，归因到 SKILL）

| 优先级 | 问题现象 | SKILL 根因 | 出现题目 |
|--------|---------|-----------|---------|
| P0 | {用户侧观察到的现象} | {对应 SKILL 的哪个模块/逻辑有缺陷} | {题号} |
| P1 | ... | ... | ... |
