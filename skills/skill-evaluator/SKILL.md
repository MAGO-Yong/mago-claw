...
Agent Native 核心问题：
- [P0] AN-1: {问题描述} → 建议：{一句话改造建议}
- [P1] AN-4: {问题描述} → 建议：{一句话改造建议}
...
> **格式说明**：每条问题后必须跟 `→ 建议：{具体可操作的改法}`，如"统一 except 块返回 JSON：`{error_code, action, retry}`"，避免用户不知道如何着手。建议控制在 1 句话以内，复杂改造点指向报告正文的对应章节。
