...
|--------|------|------|------------|------|-------|
| P0 | AN-1 | {问题描述} | {具体改法} | {低/中/高} | 🔴/{🟠}/{🟡} |
| P0 | AN-2 | {问题描述} | {具体改法} | {低/中/高} | 🔴/{🟠}/{🟡} |
| P1 | AN-4 | {问题描述} | {具体改法} | {低/中/高} | 🔴/{🟠}/{🟡} |

> 紧迫度说明：🔴 影响日常使用，建议尽快修复 | 🟠 偶发场景受影响，可计划修复 | 🟡 不影响日常使用，按需优化
> 难度说明：低（改 1-2 行）| 中（改 1 个函数/模块）| 高（涉及架构或多处联动）
| P2 | AN-7 | {问题描述} | {具体改法} |
...
|--------|------|---------|-----------|---------|------|-------|
| P0 | Agent Native | {现象} | {AN 子项} | {具体怎么改} | {低/中/高} | 🔴 影响日常使用 |
| P0 | 运行表现 | {现象} | {SKILL 的哪个模块/逻辑} | {具体怎么改} | {低/中/高} | 🔴 影响日常使用 |
| P1 | Agent Native | {现象} | {AN 子项} | {具体怎么改} | {低/中/高} | 🟠 偶发场景受影响 |
| P1 | 运行表现 | {现象} | {SKILL 的哪个模块/逻辑} | {具体怎么改} | {低/中/高} | 🟠 偶发场景受影响 |
| P2 | Agent Native | {现象} | {AN 子项} | {具体怎么改} | {低/中/高} | 🟡 按需优化 |
...
- [P0] AN-1: {问题描述}
  → 建议：{一句话改造建议}
  → 难度：{低/中/高} | 紧迫度：{🔴 影响日常使用，建议尽快修复 / 🟠 偶发场景受影响，可计划修复 / 🟡 不影响日常使用，按需优化}
...
> **格式说明**：
> - `建议`：具体可操作的改法，1 句话以内，复杂改造点可指向报告正文章节
> - `难度`：低（改 1-2 行）/ 中（改 1 个函数/模块）/ 高（涉及架构或多处联动）
> - `紧迫度`：🔴 日常查询/操作会触发此问题 → 建议尽快修复；🟠 仅边界场景触发，不影响主流程；🟡 纯优化项，不影响使用
